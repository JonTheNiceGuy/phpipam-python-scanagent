...
logging.info(f"Version: 1.0.10 Author: Jon Spriggs jon@sprig.gs")
...
logging.debug(f"Server: {server}, api_client: {api_client}, api_token: {api_token}, agent_code: {agent_code}, sleep_duration: {sleep_duration}, always_update_seen_hosts: {always_update_seen_hosts}")
...
def getFromPhpIpam(
    ...
    if 'data' in return_data:
        logging.debug(return_data['data'])
...
while True:
    ...
    if scanagentId > 0:
        logging.debug(f"Scan agent detected: {agent_code}.\n")
        logging.debug(f"Starting run {nowTime()}.\n")
        ...
        for subnet in subnets:
            ...
            if str_scanAgent == str_scanagentId:
                logging.info(
                ...
                if str_pingSubnet == str('1') and scan_now:
                    nameservers = ' -n'
                    ...
                    logging.info(f"Scanning subnet {cidr} '{subnet['description']}'.")
                    add_online = 0
                    update_online = 0
                    update_offline = 0
                    update_removed = 0
                        ...
                        for ip in hosts_list:
                            ...
                            if 'state' in status['state'] and status['state']['state'] == 'up' and status['state']['reason'] != 'reset':
                                ...
                                if ip_found:  # Update
                                    ...
                                    logging.info(createPhpIpam(server, api_client,
                        ...
                        if addresses_list is not None and len(addresses_list) > 0:
                            for ip in addresses_list:
                                ...
                                if (remove_old_hosts == '1' and
                                    ...
                                    logging.info(deletePhpIpam(server, api_client,
                                    ...
                                    logging.info(updatePhpIpam(server, api_client,
                    ...
                    if update_online + add_online + update_offline + update_removed > 0:
                        ...
                        logging.info(f"No changes at {thisTime}")
